...
object TickType extends Enumeration {
  ...
  lazy private val toIntsMap: Map[TickType, Int] = fromIntsMap.map{case (t, int) => (int, t)}
  lazy private val fromIntsMap: Map[Int, TickType] = this.values.flatMap {
    case tick: Tick => Some(tick.asInstanceOf[Tick].code, tick)
    case _ => None
  }.toMap
